...
- Added: README section "End-of-Life — replacement by RcCustomFields" with six steps in a safe order, the way back and three symptoms for the failure case; the inventory query was verified against real data.
- Added: The plugin main class now states what it works against and that it is a transitional solution.
...
- Changed: Plugin interaction protocol and README document the new event contract and the simpler extension path.
